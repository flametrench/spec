...
**Every per-notification operation takes the authenticated `recipient_usr_id` and scopes by it internally** — `getNotification(recipient_usr_id, id)`, `markRead(recipient_usr_id, id)`, `markUnread(recipient_usr_id, id)`, `dismiss(recipient_usr_id, id)`, `countUnread(recipient_usr_id)`, and `listNotifications(recipient_usr_id, …)`. The recipient is a required argument, not an adopter-side filter: the SDK MUST constrain the lookup to `recipient_usr_id = <authenticated recipient>` so a caller cannot reach another user's notification by id.

### Access — strictly recipient-scoped, SDK-enforced (normative)

A notification is **not** a general authz object. Access is **strictly bound to the recipient** and **enforced by the SDK** (not delegated to the adopter): every per-notification operation takes the authenticated `recipient_usr_id` (above) and the implementation MUST scope the lookup to it — `get`, `listNotifications`, `countUnread`, `markRead`, `markUnread`, and `dismiss` operate **only** on notifications whose `recipient_usr_id` equals the supplied recipient. An operation whose target notification belongs to a different recipient (or does not exist) MUST resolve to the **same not-found error** — indistinguishable, per §Errors and the tenancy-non-inference rule below; there is no cross-recipient read path in the primitive.

This is deliberately **not** the "SDK does not gate; adopter does" pattern used for cross-tenant reads (`listOrgs`/`listUsers`/`check`): a notification has an **intrinsic owner**, and a per-id `get` with adopter-side gating is the classic IDOR footgun (the `get`-vs-`list` asymmetry — an adopter who scopes `list` but forgets to scope `get` leaks). Secure-by-default wins: the spec enforces it. Administrative or "see all notifications in an org" views are out of scope here; if a genuine read-any need ever appears it is a **separate, explicitly-gated primitive** (the [ADR 0025](./0025-list-orgs.md) gated-system-level-read precedent), never a relaxation of this scoping.
...
**Recipient-scope / existence non-disclosure (NOT an input error).** Per §"Access — strictly recipient-scoped" and §"Tenancy non-inference", `get`/`markRead`/`markUnread`/`dismiss`/`countUnread`/`listNotifications` targeting a notification the caller does **not** own MUST be **indistinguishable from targeting a non-existent one**: both raise the **same** not-found error — **`NotFoundError`** (pinned so the cross-recipient vector is conformance-enforceable; the *same* type for both the foreign-recipient and the genuinely-non-existent case is the load-bearing requirement) — with no **presence / count / error-code / error-message-or-payload** differential that could reveal a foreign notification's existence. Implementations MUST NOT raise a distinct "forbidden/denied" error for the cross-recipient case — that differential would itself be the existence oracle the non-inference rule forbids.

**Existence/ownership check takes precedence over state-machine validation.** An operation on a notification the caller does not own (or that does not exist) MUST resolve to the not-found error **before** any `PreconditionError` is evaluated — otherwise, e.g., `dismiss` on a foreign, already-`dismissed` notification would surface the terminal-state `PreconditionError` and leak both its existence *and* its state. (The notify analog of ADR 0016's lifecycle-vs-secret ordering, inverted: here existence *is* sensitive, so it resolves first.)

**Wall-clock API timing is out of scope at the contract layer** — exactly as for audit ([ADR 0019](./0019-audit-primitive.md) §"Timing is scoped to the audit stream, not the API"). A recipient-scoped `SELECT … WHERE id = ? AND recipient_usr_id = ?` has inherent "exists-but-not-yours" vs "does-not-exist" timing skew with no cheap decoy to flatten it (even the PAT/password Argon2id decoy carries a documented residual). So response **indistinguishability** above is the normative MUST; constant-time *wall-clock* response is an **adopter security-hardening** concern (SHOULD / best-effort; see [`docs/security.md`](../docs/security.md)), not a contract-level MUST — notify does not promise a stronger timing guarantee than audit for the same oracle class.

(This is the recipient-scoped instance of the [ADR 0019](./0019-audit-primitive.md) cross-scope non-disclosure discipline.)
