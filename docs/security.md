...
- **Constant-time password verify, with a documented residual.** `verifyPassword` runs a fixed dummy Argon2id verify on the row-miss path so "no such credential" and "wrong password" take the same wall time (security-audit-v0.3 H2). This equalization holds exactly only while all stored hashes share the floor params; its above-floor-params limitation and migration guidance are in [§Timing-equalization residual (decoy verify)](#timing-equalization-residual-decoy-verify) — the password path is the primary one this matters for.
...
- **System-wide / cross-tenant enumeration reads are adopter-gated.** `listUsers` (install-wide; [ADR 0015](../decisions/0015-list-users.md)) and `listOrgs` (cross-tenant; [ADR 0025](../decisions/0025-list-orgs.md)) enumerate the entire user / org table and enforce **no** authorization at the SDK layer — the broadest-blast-radius case of the spec-wide "SDK does not gate; adopter does" pattern. A missing gate on `listOrgs` discloses the **entire customer roster cross-tenant** (`id`/`status`/`name`/`slug`/timestamps per org — a searchable tenant directory, plus churn/billing signal via `status` and install growth via timestamps); a missing gate on `listUsers` discloses every account install-wide. Gate the call site to a system/install-admin caller — canonical shape `check(subject, system_admin, system_<install_id>)`; end-user / tenant-scoped routes MUST NOT reach them, and server-side fan-out callers (e.g. scheduler) MUST NOT be exposed on any tenant-scoped surface. **Conformance cannot catch a missing gate** — the route-layer gate is the only control. `listOrgs` is the single sanctioned exception to the general cross-tenant non-disclosure posture (most rigorously specified, for the audit stream, in [ADR 0019](../decisions/0019-audit-primitive.md)). Note `status='revoked'` keeps revoked orgs enumerable with full metadata — revoke is a lifecycle state, not erasure; adopters with tenant-deletion / data-protection obligations purge separately.
...
- **Bearer wire format `pat_<32hex>_<base64url>`.** The leading `pat_` prefix is normative and lets `resolveBearer` dispatch by scheme without first-row probing — a malformed bearer is rejected on length/charset alone, before any DB read. The token MUST be parsed at the fixed boundary — the ID is the 32 hex chars after `pat_` and the secret is everything after the **second** `_` (ADR 0016 §"Verification semantics" step 2). Implementations MUST NOT split on the first or last `_`: the base64url secret alphabet includes `_`, so a last-underscore split mis-slices secrets that contain it.
- **Bearer dispatch order (normative).** `resolveBearer` inspects prefixes in a fixed order — `pat_` → `verifyPatToken`, `shr_` → `verifyShareToken`, otherwise → `verifySessionToken` (v0.2 opaque session tokens carry no prefix and are the fallthrough). The prefixes are mutually exclusive, so the order is a formality today; it is pinned so SDKs agree and so any future prefixed bearer type is **appended** to the chain, never inserted ahead of `pat_`/`shr_`.
- **Constant-time secret verification.** Implementations MUST run Argon2id verify even when the row lookup misses (using a fixed dummy PHC hash) so that "row missing" and "wrong secret" take the same wall time. This branch MUST also cover the structurally-valid-but-non-UUIDv7 id path, not only the missing-row path (security-audit-v0.3 H2). **The dummy hash is pinned for cross-SDK conformance — every SDK MUST use this exact string** (a self-generated hash with different parameters has a different verify time and reintroduces the oracle):
  ```
  $argon2id$v=19$m=19456,t=2,p=1$779z4UHkLWR4w0TEo9gcHg$Gz0+nGnpokhsKi1cPlx8i74FBN1Nq0OURZ3xso1AHMU
  ```
  It verifies against plaintext `correcthorsebatterystaple` at the spec-floor params; `conformance/fixtures/identity/argon2id.json` is the executable source.
  - **Residual limitation.** This decoy equalization has a documented above-floor-params residual (it holds exactly only while all stored hashes share the floor param set). It is the same mechanism and the same caveat for `verifyPassword`, so it is described once, cross-cutting, in [§Timing-equalization residual (decoy verify)](#timing-equalization-residual-decoy-verify).
- **PAT secret length cap (MUST).** The wire regex `^pat_[0-9a-f]{32}_[A-Za-z0-9_-]{43,}$` has no upper bound, so implementations MUST reject a secret segment longer than **256 characters** with a cheap length check **before** dispatching to Argon2id. A real secret is 43 chars (32 bytes base64url); 256 leaves generous margin while bounding Argon2id-DoS amplification against a known `pat_id` (security-audit-v0.3 H6). All v0.3 SDKs enforce this cap.
...
**Mitigation.** Equalization is exact only when *all stored hashes share one param set*. Raising params above the floor therefore means: rehash all existing stored secrets to the new params (homogenize the population) **and** re-pin the decoy to match, as one migration. During the migration window the population is mixed and no single decoy can cover it — rows still at the weaker params remain measurably faster than a missing row, so treat the enumeration oracle as **present for the not-yet-rehashed subset until the migration completes.** Simply bumping the decoy without rehashing the stored population does not close it. Applies to all SDK families including Go. (security-audit-v0.3 timing review.)
