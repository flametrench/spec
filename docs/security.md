...
- **Constant-time secret verification.** Implementations MUST run Argon2id verify even when the row lookup misses (using a fixed dummy PHC hash) so that "row missing" and "wrong secret" take the same wall time. This branch MUST also cover the structurally-valid-but-non-UUIDv7 id path, not only the missing-row path (security-audit-v0.3 H2). **The dummy hash is pinned for cross-SDK conformance — every SDK MUST use this exact string** (a self-generated hash with different parameters has a different verify time and reintroduces the oracle):
  ...
  - **Residual limitation (documented, accepted for v0.3).** The decoy equalizes timing only while the *stored* hash uses the floor params (`m=19456, t=2, p=1`) — the same params as the pinned dummy hash. An adopter that migrates in an **above-floor** hash (higher memory/time, e.g. after a hardening upgrade) makes the real verify slower than the floor-param decoy, so "row present" becomes measurably slower than "row missing" and a **coarser timing oracle** re-emerges. This is acceptable for v0.3 — the floor is the spec default and the cross-SDK conformance target — but it is a known limitation, not a guarantee. **Mitigation.** Equalization is exact only when *all stored hashes share one param set*. Raising params above the floor therefore means: rehash all existing stored secrets to the new params (homogenize the population) **and** re-pin the decoy to match, as one migration. During the migration window the population is mixed and no single decoy can cover it — rows still at the weaker params remain measurably faster than a missing row, so treat the enumeration oracle as **present for the not-yet-rehashed subset until the migration completes.** Simply bumping the decoy without rehashing the stored population does not close it. Flagged by the security-audit-v0.3 timing review; applies to all SDK families including Go.
