...
- **SDKs**: Python / Node / PHP / Java each tagged at `v0.2.0-rc.x` across the family. Current versions: `ids@v0.2.0-rc.{2,3}`, `authz@v0.2.0-rc.{4,5}` (PHP rc.5; others rc.4), `tenancy@v0.2.0-rc.{5,6}` (PHP rc.6; others rc.5), `identity@v0.2.0-rc.6` (Node artifact at rc.7 — same spec contract, republished after a broken rc.6 publish). Packagist tracks tags directly. npm: `@flametrench/{ids,identity}` are current via the `rc` dist-tag; `tenancy` and `authz` lag local by one RC each. PyPI and Maven Central are bootstrapping (org / credential approvals pending). Always verify the registry directly: `npm view @flametrench/<pkg> versions --json` (note the plural — singular `version` returns only `latest` and hides RCs published under `rc`).
