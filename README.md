...
The spec is at `v0.3.0`, stable. The SDK matrix is now **five families** (`ids` / `identity` / `tenancy` / `authz` in each); the four original families (Python, Node, PHP, Java) are at `v0.3.0` and Go — the fifth, added per ADR 0018 — has shipped (latest tag `v0.3.2`). The v0.2 work splits across nine ADRs (0007–0015) and one backport:
...
**User display name + updateUser (ADR 0014).** `User` carries an optional `display_name` (spec-permissive — no length cap, no required normalization; adopters layer their own rules) with partial-update semantics through a new `updateUser` operation.
...
- **v0.3 spec**: stable, tagged `v0.3.0` (2026-05-15). The **Go SDK family addition** (ADR 0018, Accepted) has **shipped** — Go is the 5th SDK family and the lockstep release hold is lifted (latest Go tag `v0.3.2`). Surface: personal access tokens (ADR 0016) — non-interactive bearer credentials for CLI / CI / server-to-server use, with prefix-routed verification (`pat_…` / `shr_…` / session) and a new `auth.kind` audit discriminator; Postgres-backed rewrite-rule evaluation (ADR 0017) — `PostgresTupleStore.check()` accepts the same `rules` option as `InMemoryTupleStore`, retiring the v0.2 in-memory-shadow workaround. The v0.3 security audit (32 findings) is closed: 22 fixed in code, 7 spec-documented, 2 explicit v0.4 deferrals — see [`docs/security-audit-v0.3.md`](docs/security-audit-v0.3.md). Migration guidance: [`docs/migrating-to-v0.3.md`](docs/migrating-to-v0.3.md).
- **v0.4 spec**: in development. The first four v0.4 primitive ADRs are ratified (Status: Proposed), upstreamed from SiteSource (first adopter): audit (ADR 0019), file-metadata (ADR 0020), feature flags (ADR 0021), and notifications (ADR 0022) — each promotes its `aud` / `file` / `flag` / `not` prefix in `docs/ids.md`. These are deliberate surface expansions (audit/notifications/flags/file were out of scope through v0.3). Audit is the keystone: it fulfills ADR 0016's `auth.kind` forward reference, and file-metadata, flags, and notifications each emit `aud` events on its schema. Normative once v0.4 releases; SDK implementation + conformance fixtures across the five families and per-capability docs (`docs/audit.md`, …) are follow-ons.
- **SDKs**: Python / Node / PHP / Java are at `v0.2.0`, and Go is at `v0.3.1`, across the family (`ids`, `identity`, `tenancy`, `authz`). The spec is at v0.3.0 (with Go as the 5th SDK family); SDK version tracks are independent. PyPI Python publish is guarded pending Trusted Publisher setup (no long-lived tokens). Maven Central Java publish is pending Sonatype token regen. Packagist PHP and npm Node publish completed. Go publishes via `go get` directly from the GitHub tag (no central registry). Always verify a registry directly before quoting state: `npm view @flametrench/<pkg> versions --json` (note the plural — singular `version` returns only the `latest` dist-tag). The release checklist at `docs/release-checklist.md` is the canonical pre-publish process.
...
- **Conformance suite**: 29 fixture files (27 v0.1/v0.2 + 2 v0.3 PAT fixtures), executed by all five SDK families.
