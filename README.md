...
- **SDKs**: Python / Node / PHP / Java are at `v0.3.0` and Go at `v0.3.2`, across the family (`ids`, `identity`, `tenancy`, `authz`). v0.3.0 is no longer gated on the Go family, which has shipped. PyPI and Maven Central are still bootstrapping (org / credential approvals pending) and will publish v0.3.0 once unblocked. Go publishes via `go get` directly from the GitHub tag (no central registry). Always verify a registry directly before quoting state: `npm view @flametrench/<pkg> versions --json` (note the plural — singular `version` returns only the `latest` dist-tag). The release checklist at `docs/release-checklist.md` is the canonical pre-publish process.
